...
def test_intersections_and_geometries(paths, lsi):
    ...
    assert len(load_ann(paths, 341, 0).paragraphs) == 1
    assert len(load_ann(paths, 342, 0).paragraphs) == 1
    assert len(load_ann(paths, 343, 0).paragraphs) == 2
    assert len(load_ann(paths, 344, 1).paragraphs) == 2
